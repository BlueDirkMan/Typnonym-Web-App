...
let currentScore = 0
let wordIndex = 0
let wordIndexTracking = []
let currentVocab = "placeholder"
let synonymIndex = 0
const synonymBank = []
...
const gameLoop = async function () {
    ...
    async function newSynonym() {
        ...
        const currentSynonym = synonymBank[synonymIndex]
        const currentSynonymSplit = currentSynonym.split("")
        currentSynonymSplit.forEach(character => {
            const synonymCharacterSpan = document.createElement('span')
            synonymCharacterSpan.innerText = character
            synonymDisplay.appendChild(synonymCharacterSpan)
        })
        synonymIndex += 1
    }
    ...
    const typingBoxFunction = function (event) {
        ...
        const typeWordSplit = typedWord.split("")
        const synonymLetterArray = synonymDisplay.querySelectorAll('span')
        let correctWord = true;
        synonymLetterArray.forEach((synonymLetterLetterSpan, index) => {
            const currentTypedCharacter = typeWordSplit[index]
            if (currentTypedCharacter == null) {
                synonymLetterLetterSpan.classList.remove('letter-correct')
                synonymLetterLetterSpan.classList.remove('letter-incorrect')
                correctWord = false;
            } else if (currentTypedCharacter === synonymLetterLetterSpan.innerText) {
                synonymLetterLetterSpan.classList.add('letter-correct')
                synonymLetterLetterSpan.classList.remove('letter-incorrect')
            } else {
                synonymLetterLetterSpan.classList.add('letter-incorrect')
                synonymLetterLetterSpan.classList.remove('letter-correct')
                correctWord = false;
            }
        })
    ...
    typingBox.addEventListener('input', typingBoxFunction)
    ...
    const interval = setInterval(function () {
        ...
        if (timerTime < 0 || gameRestart === true) {
            ...
            typingBox.removeEventListener('input', typingBoxFunction)
            typingInput.addEventListener('input', startFunction)
            typingInput.value = null;
            instruction.classList.toggle('active')
            wordDisplay.classList.toggle('active')
            ...
            // Reset Score & WPM & Increase Round Number
            currentScore = 0
            wpmDisplay.innerText = 0
            scoreDisplay.innerText = 0
            characterTyped = 0
            roundNumber.innerHTML = parseInt(roundNumber.innerHTML, 10) + 1
...
// Located at the bottom to make the rest of the code more readable
const wordBank = [
    // Copied from apostrophecms /random-words which borrowed from xkcd password generator which borrowed 
    // it from wherever
...
const randomNumberFromArray = function(array) {
